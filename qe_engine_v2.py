...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--auto-breadth",
        action="store_true",
        help="Fetch breadth ratio automatically from Kabutan. Falls back to --breadth if failed.",
    )
    parser.add_argument(
        "--min-score",
        type=float,
...
def fetch_kabutan_breadth() -> float | None:
    """
    株探から市場内部の騰落感を取得。
    戻り値:
        値上がり銘柄数 / 値下がり銘柄数
    ...
    try:
        ...
        up_match = re.search(r"値上がり[^0-9]*([\d,]+)", html)
        down_match = re.search(r"値下がり[^0-9]*([\d,]+)", html)

        if not up_match or not down_match:
            return None

        up = int(up_match.group(1).replace(",", ""))
        down = int(down_match.group(1).replace(",", ""))
...
def build_future(
    ...
    future["Mercury"] = future["MercuryScore"].apply(
        lambda x: "逆行中" if x != 0 else ""
    )
...
def main() -> None:
    ...
    # BreadthGuard 用の騰落レシオ。
    # --auto-breadth が指定された場合は株探から自動取得し、
    # 失敗した場合は --breadth の手入力値へフォールバックする。
    current_breadth = args.breadth

    if args.auto_breadth:
        fetched_breadth = fetch_kabutan_breadth()
        if fetched_breadth is not None:
            current_breadth = fetched_breadth
            print(f"Auto breadth fetched: {current_breadth:.3f}")
        else:
            print(f"Auto breadth failed. Fallback to --breadth: {current_breadth:.3f}")
    ...
    print(f"Breadth     : {settings.current_breadth:.3f}")
    print(f"Breadth th. : {settings.breadth_threshold:.3f}")
